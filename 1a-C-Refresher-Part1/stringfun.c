...
// The purpose of providing prototypes for the functions in this code
// module is for forward declaration and type checking. C complies code from top to
// bottom, and if we call a function before it is defined, the compiler will not know
// what the function is. Prototypes also helps the compiler to perform type checking.
// It specifies the number of arguments, the type of arguments, and the return type.
...
void word_print(char *str)
{
    ...
    int last_char_idx = len - 1; // index of last char - 1;
